...
# To change
    ...
    encoder = Bidirectional(LSTM(embedding_size, 
                return_state=True,
    ...
    decoder_lstm = Bidirectional(LSTM(embedding_size, 
                return_sequences=True, 
                return_state = True,
                recurrent_dropout = 0.2,
                name='dec_lstm'
                ))
    decoder_op, _, _, _, _ = decoder_lstm(x, initial_state=encoder_states)
    decoder_outputs = TimeDistributed(Dense(1, activation='linear'), name='dec_op_td')(decoder_op)
